...
void ConeDetectorNode::filter(const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
  ...
  seg.setDistanceThreshold(0.01);       // soglia di distanza
  seg.setMaxIterations(1000);           // numero massimo di iterazioni
  seg.setNormalDistanceWeight(0.1);     // Peso della distanza rispetto alle normali
  ...
  else std::cout << "No cone detected." << std::endl;
